...
- 6.3 旋您可以通过调整'旋转角度'来改变元素的方向。系统支持从-180度到180度的调整范围。
...
- 8.18 单元格高度：当表格中包含二维码、条形码或者图片列时，您可以为这些列设定单元格高度。一旦设定，二维码、条形码和图片将按照您设定的单元格高度展示。需要注意的是，如果多列都设定了单元格高度，只会依据最大的设定值来显示。如下图所示：
...
- 12.1 首先，您需要按照 [预览打印、直接打印](ad-print.md)中的1至3步来配置预览打印信息。

- 12.2 设定信息、ApexClass和VisualforcePage文件信息如下图所示：

- 12.2.1 设定信息：模板名称选择11.2步骤中的模板编号，使用功能部分选择：'模板预览后打印'和'PDF下载'。

![Create155](../_images/zh-cn/Create155.gif)

- 12.2.2 ApexClass：代码中的dataSource就是展示在模板中的数据

![Create156](../_images/zh-cn/Create156.gif)

- 12.2.3 VisualforcePage：代码中的standardcontroller要设置为当前抽取数据的对象API名，extensions设置为12.2.2步骤的ApexClass名。

![Create157](../_images/zh-cn/Create157.gif)

- 12.3 在设置完成预览打印信息后，选择自定义业务对象作为模板数据抽取的源头。(当前演示使用的是Demo01)，选择一笔数据查看详细信息，商品 説明中存放的是模拟通过自定义对象抽取的数据。如下图所示：

![Create158](../_images/zh-cn/Create158.gif)

- 12.4 在打开自定义对象数据的详细信息画面后，您会看到右上角有一个预览按钮。点击预览按钮，您可查看带有数据的模板。点击打印，则可以将当前的模板连同数据一起打印出来。点击下载，会将当前的模板以及附带的数据导出为PDF文件。如下图所示：
